...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update existing menu item
        ...
        if (!$menuItem) {
            ...
            $menuData = [
                ...
                'is_available' => isset($_POST['is_available']) ? 1 : 0,
                'image' => $menuItem['image'] // Default to existing image
            ];
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFilePath)) {
                    ...
                    if ($menuItem['image'] && file_exists('../' . $menuItem['image'])) {
    ...
    // Toggle menu availability
        ...
        if ($menuRepo->toggleAvailability($id)) {
            $message = 'Status ketersediaan menu berhasil diperbarui';
            $messageType = 'success';
        } else {
            $message = 'Gagal memperbarui status ketersediaan menu';
            $messageType = 'danger';
        }
    }
}
                                ...
                                <?php else: ?>
                                    <?php foreach ($menuItems as $item): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($item['image']): ?>
                                                    <img src="../<?= $item['image'] ?>" alt="<?= $item['name'] ?>" class="img-thumbnail" style="max-height: 80px;">
                                                <?php else: ?>
                                                    <img src="../assets/img/no-image.png" alt="No Image" class="img-thumbnail" style="max-height: 80px;">
                                                <?php endif; ?>
                                            </td>
                                            <td><?= htmlspecialchars($item['name']) ?></td>
                                            <td><?= htmlspecialchars($item['description']) ?></td>
                                            <td>Rp <?= number_format($item['price'], 0, ',', '.') ?></td>
                                            <td><?= htmlspecialchars($item['category'] ?? 'main') ?></td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    <form method="post" style="display:inline;">
                                                        ...
                                                        <button type="submit" class="btn btn-sm btn-<?= $item['is_available'] ? 'warning' : 'success' ?>" title="<?= $item['is_available'] ? 'Tandai Tidak Tersedia' : 'Tandai Tersedia' ?>">
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-edit').click(function() {
            ...
            const available = $(this).data('available') == 1;
            ...
            $('#edit_is_available').prop('checked', available);
